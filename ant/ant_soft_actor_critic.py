...
class AntSoftActorCritic:
    ...
    def record(self, T, n, video_dir='', on_policy=False, deterministic=False):
        ...
        for i in range(n):
            self.test_env.reset()
            wrapped_env = monitor_env(self.test_env, video_dir + '_%d'%(i))
            o = wrapped_env.reset()
...
if __name__ == '__main__':
    ...
    sac1 = AntSoftActorCritic(lambda : make_env(resolve_env_id(args.env)), 
    ...
    sac2 = AntSoftActorCritic(lambda : make_env(resolve_env_id(args.env)), 
        actor_critic=core.mlp_actor_critic, 
        seed=args.seed, gamma=args.gamma, 
        ac_kwargs=dict(hidden_sizes=[args.hid]*args.l),
        logger_kwargs=logger_kwargs)
