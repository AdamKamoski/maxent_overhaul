...
def compute_states_visited_xy(env, policies, T, n, norm=[],
    ...
    for it in range(N): 
        ...
        for iteration in range(n):
            ...
            obs = get_state(env, raw_obs_from_state(env))
...
def execute_average_policy(env, policies, T, weights,
    ...
    for iteration in range(n):
        ...
        if render and iteration == 0:
            ...
            wrapped_env = monitor_env(env, video_dir)
            wrapped_env.reset()
            qpos = initial_state[:len(ant_utils.qpos)]
            qvel = initial_state[len(ant_utils.qpos):]
            wrapped_env.unwrapped.set_state(qpos, qvel)
            obs = get_state(wrapped_env, raw_obs_from_state(wrapped_env), wrapped=True)
...
def init_state(env):    
    env.unwrapped.set_state(ant_utils.qpos, ant_utils.qvel)
    state = env.unwrapped.state_vector()
    return state
...
def collect_entropy_policies(env, epochs, T, MODEL_DIR=''):
    ...
    base_states_visited_indexes = [0,5,10,15]
    states_visited_indexes = [idx for idx in base_states_visited_indexes if idx < epochs]
    if len(states_visited_indexes) == 0 and epochs > 0:
        states_visited_indexes = [0]
    ...
    for i in range(epochs):
        ...
        sac = AntSoftActorCritic(lambda : make_env(resolve_env_id(args.env)), reward_fn=reward_fn, xid=i+1,
        ...
        col3 = [round_entropy_xy, running_avg_ent_xy, 
                ...
                pct_xy, pct]
        table = tabulate(np.transpose([col1, col2, col3]), col_headers, tablefmt="grid", floatfmt=".4f")
        ...
        if len(states_visited_indexes) >= 4 and i == states_visited_indexes[3]:
