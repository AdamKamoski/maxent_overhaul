...
# python curiosity.py  --models_dir=models-Pendulum-v1/models_2018_11_29-09-48/ --env="Pendulum-v1"
...
def get_obs(state):
    if base_utils.args.env == "Pendulum-v1":
...
def execute_policy_internal(env, T, policies, state, render):
    ...
    for t in range(T):
        ...
        if (t == random_T and not render):
            random_initial_state = env.unwrapped.state
...
def execute_average_policy(env, policies, T, initial_state=[], avg_runs=1, render=False):
    ...
    for i in range(avg_runs):
        ...
        env.unwrapped.reset_state = initial_state
        state = env.reset()
