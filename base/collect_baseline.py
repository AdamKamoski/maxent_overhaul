...
def collect_entropy_policies(env, epochs, T, MODEL_DIR):
    ...
    if args.env == "Pendulum-v1":
        ...
        seed = env.unwrapped._get_obs()
    elif args.env == "MountainCarContinuous-v0":
        env.unwrapped.state = [-0.50, 0]
        seed = env.unwrapped.state
    ...
    for i in range(epochs):
        ...
        if args.env == "Pendulum-v1" and random.random() < 0.3:
            initial_state[1] = 0
...
def main():
    ...
    if args.env == "Pendulum-v1":
        env.unwrapped.max_speed = 8
        env.unwrapped.max_torque = 1
