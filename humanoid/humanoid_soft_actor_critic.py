...
class HumanoidSoftActorCritic:
    ...
    def __init__(self, env_fn, reward_fn=None, actor_critic=core.mlp_actor_critic, 
        ...
        self.obs_dim = len(self.env.unwrapped.state_vector())
    ...
    def test_agent(self, T, n=10, initial_state=[],
        ...
        for j in range(n):
            ...
            if len(initial_state) > 0:
                ...
                self.test_env.unwrapped.set_state(qpos, qvel)
                o = raw_obs_from_state(self.test_env)
    ...
    def record(self, T, n=1, video_dir='', on_policy=False, deterministic=False):
        ...
        uid = 0
        for i in range(n):
            wrapped_env = monitor_env(self.test_env, video_dir + '/%d'%(uid))
            uid = uid + 1
            o = wrapped_env.reset()
...
if __name__ == '__main__':
    ...
    sac1 = HumanoidSoftActorCritic(lambda : make_env(resolve_env_id(args.env)), 
    ...
    sac2 = HumanoidSoftActorCritic(lambda : make_env(resolve_env_id(args.env)), 
        actor_critic=core.mlp_actor_critic, 
        seed=args.seed, gamma=args.gamma, 
        ac_kwargs=dict(hidden_sizes=[args.hid]*args.l),
        logger_kwargs=logger_kwargs)
